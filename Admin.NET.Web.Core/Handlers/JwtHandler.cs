...
namespace Admin.NET.Web.Core
{
    public class JwtHandler : AppAuthorizeHandler
    {
        ...
        public override async Task HandleAsync(AuthorizationHandlerContext context)
        {
            ...
            {
                var rolesAuthorizationRequirement = context.Requirements.FirstOrDefault(t => t.GetType() == typeof(RolesAuthorizationRequirement));
                if (rolesAuthorizationRequirement != null)
                {
                    var roles = App.User?.FindFirst(ClaimConst.Roles)?.Value?.Split(',');
                    var result = (rolesAuthorizationRequirement as RolesAuthorizationRequirement).AllowedRoles.Intersect(roles);
                    if (!result.Any())
                    {
                        context.Fail(); // 授权失败
                        return;
            ...
            if (JWTEncryption.AutoRefreshToken(context, context.GetCurrentHttpContext(), 120))
            {
