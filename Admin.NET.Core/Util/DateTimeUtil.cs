...
public class DateTimeUtil
{
    ...
    public static double? GetTimeSpan(string timeStart, string timeEnd, string format, string spanType)
    {
        DateTime value;
        bool flag = !DateTime.TryParseExact(timeStart, format, CultureInfo.InvariantCulture, DateTimeStyles.None, out value);
        double? result;
        if (flag)
        {
            result = null;
        ...
        {
            DateTime dateTime;
            bool flag2 = !DateTime.TryParseExact(timeEnd, format, CultureInfo.InvariantCulture, DateTimeStyles.None, out dateTime);
            if (flag2)
            {
                result = null;
            }
            ...
            {
                bool flag3 = spanType == "Day";
                if (flag3)
                {
                    result = new double?(dateTime.Subtract(value).TotalDays);
                }
                else
                {
                    bool flag4 = spanType == "Hour";
                    if (flag4)
                    {
                        result = new double?(dateTime.Subtract(value).TotalHours);
                    }
                    else
                    {
                        bool flag5 = spanType == "Minute";
                        if (flag5)
                        {
                            result = new double?(dateTime.Subtract(value).TotalMinutes);
                        }
                        else
                        {
                            bool flag6 = spanType == "Second";
                            if (flag6)
                            {
                                result = new double?(dateTime.Subtract(value).TotalSeconds);
                            }
                            else
                            {
                                result = new double?(dateTime.Subtract(value).TotalMilliseconds);
                            }
                        }
                    }
                }
    ...
    public static double? GetMinutesSpan(string timeStart, string timeEnd, string format)
    {
        return GetTimeSpan(timeStart, timeEnd, format, "Minute");
    }
    ...
    public static double? GetHoursSpan(string timeStart, string timeEnd, string format)
    {
        return GetTimeSpan(timeStart, timeEnd, format, "Hour");
    }
}