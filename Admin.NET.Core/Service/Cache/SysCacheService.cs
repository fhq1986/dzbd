...
public class SysCacheService : IDynamicApiController, ISingleton
{
    ...
    public SysCacheService(ICache cache, IOptions<CacheOptions> cacheOptions, IServiceScopeFactory scopeFactory)
    {
        _cache = cache;
    ...
    [AllowAnonymous]
    public IEnumerable<Bdsj> TestDapper2Dm()
    {
        IList<ISort> sorts = new List<ISort>();
        sorts.Add(new Sort { PropertyName="Id",Ascending=true });
        using var scope = _scopeFactory.CreateScope();
        var _dapperRepository=scope.ServiceProvider.GetRequiredService<IDapperRepository>();
       return _dapperRepository.Context.GetPage<Bdsj>(null, sorts, 1,120);
    }
}