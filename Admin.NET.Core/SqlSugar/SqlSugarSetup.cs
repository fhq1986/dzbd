...
public static class SqlSugarSetup
{
    ...
    public static void AddSqlSugar(this IServiceCollection services)
    {
        ...
        #region 注册dapper
        var type = SqlProvider.Sqlite;
        var dbType = dbOptions.ConnectionConfigs.First(t => t.ConfigId.ToString() == SqlSugarConst.MainConfigId).DbType;
        ISqlDialect sqlDialect = new SqliteDialect();
        switch (dbType)
        {
            case SqlSugar.DbType.Oracle:
                type = SqlProvider.Oracle;
            ...
            case SqlSugar.DbType.SqlServer:
                type = SqlProvider.SqlServer;
                sqlDialect = new SqlServerDialect();
                break;
            case SqlSugar.DbType.PostgreSQL:
                type = SqlProvider.Npgsql;
                sqlDialect = new PostgreSqlDialect();
                break;
            case SqlSugar.DbType.Sqlite:
                type = SqlProvider.Sqlite;
                sqlDialect = new SqliteDialect();
                break;
            default:
                type = SqlProvider.Dm;
                sqlDialect = new DmDialect();
                break;
        }

        services.AddDapper(dbOptions.ConnectionConfigs.First(t => t.ConfigId.ToString() == SqlSugarConst.MainConfigId).ConnectionString, type, () => {
            var sqlconfig = new DapperExtensionsConfiguration(typeof(AutoClassMapper<>), new List<Assembly>(), sqlDialect);
            var sqlGenerator = new SqlGeneratorImpl(sqlconfig);
            DapperExtensions.DapperExtensions.Configure(sqlconfig);
            DapperExtensions.DapperExtensions.InstanceFactory = (conf) => {
                return new DapperImplementor(sqlGenerator);
            };
            DapperAsyncExtensions.Configure(sqlconfig);
            DapperAsyncExtensions.InstanceFactory = (conf) => {
                return new DapperAsyncImplementor(sqlGenerator);
            };
    ...
    private static void InitDatabase(SqlSugarScope db, DbConnectionConfig config)
    {
        ...
        {
            if (!(config.DbType == SqlSugar.DbType.Oracle || config.DbType == SqlSugar.DbType.Dm))
        ...
        if (config.TableSettings.EnableInitTable)
        {
            ...
            {
                if (entityType==typeof(SysUserInfo)
                    || entityType.Name.Substring(0, 3) != "Sys")
                {
                    continue;
                }
