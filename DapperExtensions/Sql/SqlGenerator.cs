...
namespace DapperExtensions.Sql
{
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        private IEnumerable<IColumn> GetColumns(Table table)
        {
            ...
            var columnIndex = 0;
            foreach (var p in table.EntityType.GetProperties())
            {
               if(map.Properties.Any(t=>t.Name==p.Name))
               {
                    var customAttributes=p.GetCustomAttributes().FirstOrDefault();
                    if(customAttributes!=null)
                    {
                        var isIgnore=customAttributes.GetType().GetProperty("IsIgnore");
                        if(isIgnore!=null)
                        {
                            map.Properties.First(t => t.Name == p.Name).Ignored = bool.Parse((isIgnore.GetValue(customAttributes).ToString()));
                        }
                    }
               }
            }
